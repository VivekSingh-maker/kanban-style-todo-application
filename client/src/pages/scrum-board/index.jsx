...
function ScrumBoardPage() {
  ...
  return (
    <Fragment>
      <div className="kanbanbgpage grid grid-cols-5 gap-4 h-full">
        {scrumBoardOptions.map((item) => (
          <div
            className="kanbanseg border border-[#333333] rounded overflow-auto"
            key={item.id}
            onDrop={(event) => onDrop(event, item.id)}
            onDragOver={(event) => event.preventDefault()}
          >
            <div className="kanbanbg px-1 py-3 text-center bg-black border-none mb-3">
              <h3 className="kanbantx text-2xl font-extrabold text-white">
                {item.label}
              </h3>
            </div>
